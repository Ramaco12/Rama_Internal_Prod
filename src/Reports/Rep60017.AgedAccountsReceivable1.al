report 70107 "Aged Accounts Receivable1"
{
    DefaultLayout = RDLC;
    RDLCLayout = '.src\Layout\AgedAccountsReceivable1.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Aged Accounts Receivable_RAMA ';
    ...
    dataset
    {
        dataitem(Header; "Integer")
        {
            ...
            column(AmountFCYCaptionLbl; AmountFCYCaptionLbl)
            {
            }
            column(AmountLCYCaptionLbl; AmountLCYCaptionLbl)
            {
            }
            ...
            dataitem(Customer; Customer)
            {
                ...
                column(CustPostGroupCaption; Fieldcaption("Customer Posting Group"))
                {
                }
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(TempCustLedgEntryLoop; "Integer")
                    {
                        ...
                        column(CustPostGroup; Customer."Customer Posting Group")
                        {
                        }
                        ...
                        begin
                            if Number = 1 then begin
                                ...
                                if TempCustLedgEntry.Next() = 0 then CurrReport.Break();
                            ...
                            if DetailedCustomerLedgerEntry.FindSet(false, false) then
                                repeat
                                    if (DetailedCustomerLedgerEntry."Entry Type" = DetailedCustomerLedgerEntry."Entry Type"::"Initial Entry") and (CustLedgEntryEndingDate."Posting Date" > EndingDate) and (AgingBy <> AgingBy::"Posting Date") then
                                        if CustLedgEntryEndingDate."Document Date" <= EndingDate then
                                            DetailedCustomerLedgerEntry."Posting Date" := CustLedgEntryEndingDate."Document Date"
                                        else
                                            if (CustLedgEntryEndingDate."Due Date" <= EndingDate) and (AgingBy = AgingBy::"Due Date") then DetailedCustomerLedgerEntry."Posting Date" := CustLedgEntryEndingDate."Due Date";
            ...
            dataitem(CurrencyTotals; "Integer")
            {
                ...
                begin
                    if Number = 1 then begin
                        ...
                        if TempCurrency2.Next() = 0 then CurrReport.Break();
                    Clear(AgedCustLedgEntry);
                    TempCurrencyAmount.SetRange("Currency Code", TempCurrency2.Code);
                    if TempCurrencyAmount.FindSet(false, false) then
                        repeat
                            if TempCurrencyAmount.Date <> DMY2Date(31, 12, 9999) then
                                AgedCustLedgEntry[GetPeriodIndex(TempCurrencyAmount.Date)]."Remaining Amount" := TempCurrencyAmount.Amount
                            else
    ...
    var
        ...
        PL6: DateFormula; //Added
        GLSetup: Record "General Ledger Setup";
        CustLedgEntryEndingDate: Record "Cust. Ledger Entry";
        TotalCustLedgEntry: array[8] of Record "Cust. Ledger Entry";
        GrandTotalCustLedgEntry: array[8] of Record "Cust. Ledger Entry";
        AgedCustLedgEntry: array[9] of Record "Cust. Ledger Entry";
        ...
        AmountFCYCaptionLbl: Label 'FCY Amount';
        AmountLCYCaptionLbl: Label 'LCY Amount';
    ...
    var
        ...
        i: Integer;
    begin
        if not Evaluate(PeriodLength2, StrSubstNo(Text032Txt, PeriodLength)) then Error(EnterDateFormulaErr);
    ...
    begin
        if Format(PL2) <> '' then begin
            if not Evaluate(PeriodLength2, StrSubstNo(Text032Txt, PL2)) then //begin
    ...
    begin
        if Format(PL3) <> '' then begin
            if not Evaluate(PeriodLength2, StrSubstNo(Text032Txt, PL3)) then //begin
                Error(EnterDateFormulaErr);
            if AgingBy = AgingBy::"Due Date" then begin
                PeriodEndDate[1] := DMY2Date(31, 12, 9999);
                PeriodStartDate[1] := EndingDate + 1;
    ...
    begin
        if Format(PL4) <> '' then begin
            if not Evaluate(PeriodLength2, StrSubstNo(Text032Txt, PL4)) then //begin
                Error(EnterDateFormulaErr);
            if AgingBy = AgingBy::"Due Date" then begin
                PeriodEndDate[1] := DMY2Date(31, 12, 9999);
                PeriodStartDate[1] := EndingDate + 1;
    ...
    begin
        if Format(PL5) <> '' then begin
            if not Evaluate(PeriodLength2, StrSubstNo(Text032Txt, PL5)) then //begin
                Error(EnterDateFormulaErr);
            if AgingBy = AgingBy::"Due Date" then begin
                PeriodEndDate[1] := DMY2Date(31, 12, 9999);
                PeriodStartDate[1] := EndingDate + 1;
    ...
    begin
        if Format(PL6) <> '' then begin
            if not Evaluate(PeriodLength2, StrSubstNo(Text032Txt, PL6)) then //begin
                Error(EnterDateFormulaErr);
            if AgingBy = AgingBy::"Due Date" then begin
                PeriodEndDate[1] := DMY2Date(31, 12, 9999);
                PeriodStartDate[1] := EndingDate + 1;
    ...
    begin
        ...
        if TempCurrency.Get('') and (TempCustLedgEntry."Currency Code" = GLSetup."LCY Code") then exit;
        if TempCurrency.Get(GLSetup."LCY Code") and (TempCustLedgEntry."Currency Code" = '') then exit;
